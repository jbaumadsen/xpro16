const Board = () => {
  ...
  let statusText = ['Player O', 'Player X', 'No Winner Yet'];
  let status = `Winner is ${statusText[winner]}`;
  ...
  return (
    <div className="game-board">
      ...
      <div id="info">
        ...
        */}
        <h1 id="turn">Next Play: Player {xO[player]}</h1>
        <h1>{status}</h1>
      </div>
    </div>
  );
    ...
    <button
      ...
      onClick={(e) => {
        if(!filled && winner == 2){
          ...
          e.target.className = playerColor[player];
        }
      }}
    >
      <h1 >{mark[tik]}</h1>
...
const checkForWinner = (gameState) => {
  ...
  let currentPlayer = gameState[gameState.length - 1].player;
  let p0 = gameState.filter((item) => {
    if (item.player == currentPlayer) return item;
  });
  ...
  if (win0.length > 0) return currentPlayer;
  return 2;
};
